...
export function PlanFormHeader({ isEdit = false }: PlanFormHeaderProps) {
  return (
        ...
        <View className="relative z-10">
          ...
          <View className="flex-row items-center">
            <View className="w-10 h-10 bg-white/20 rounded-xl items-center justify-center mr-2.5">
              <Ionicons name={isEdit ? "create-outline" : "add-circle-outline"} size={20} color="#FFFFFF" />
            </View>
            <View className="flex-1">
              <Text className="text-xl font-bold text-white">
                {isEdit ? 'Edit Weekly Plan' : 'Create New Weekly Plan'}
              </Text>
              <Text className="text-xs font-medium text-white/80">
                {isEdit ? 'Update your weekly schedule' : 'Plan your dinners for the week'}
              </Text>
            </View>
          </View>
        </View>
